...
export default function HomeFeaturedCarousel({ items }: HomeFeaturedCarouselProps) {
  ...
  return (
    <div className="relative">
      ...
      >
        {items.map((item) => (
          <div key={item.id} className="snap-start shrink-0 w-[calc(33.333%-11px)] min-w-[280px] flex">
            <Link href={`/${item.type}/${item.slug}`} className="group w-full">
              <div className="bg-white border border-light-border hover:border-brand transition-colors overflow-hidden flex flex-col h-full">
                ...
                <div className="p-3 flex-1 flex flex-col">
                  <h3 className="font-semibold text-sm group-hover:text-brand transition-colors line-clamp-2">
                    {item.title}
                  </h3>
                  {item.summary && (
                    <p className="text-light-muted text-xs mt-1 line-clamp-2 flex-1">
                      {item.summary}
                    </p>
                  )}
