...
function NewsCard({ article }: { article: NewsArticle }) {
  return (
    <Link href={`/news/${article.slug}`}>
      <div className="bg-white border border-light-border hover:border-brand transition-colors overflow-hidden flex flex-col h-full cursor-pointer group">
        ...
        <div className="p-4 flex-1 flex flex-col">
          ...
          <div className="mt-3 space-y-2 text-sm">
            <div className="flex items-center gap-3 flex-wrap">
              {article.rssFeed && (
                <span className="text-xs text-light-muted bg-light-surface px-2 py-0.5 rounded">via {article.rssFeed.name}</span>
              )}
