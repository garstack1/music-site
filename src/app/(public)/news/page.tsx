...
type FeedItem = (NewsArticle & { _isEditorial?: false }) | EditorialTeaser;
...
function EditorialShareButtons({ post, section }: { post: EditorialTeaser; section: string }) {
  const url = typeof window !== "undefined"
    ? `${window.location.origin}${section}/${post.slug}`
    : "";
...
function EditorialTeaserCard({ post }: { post: EditorialTeaser }) {
  const section = EDITORIAL_TYPE_LINKS[post.type] || "/festivals";
  const href = `${section}/${post.slug}`;
  const colour = EDITORIAL_TYPE_COLOURS[post.type] || "bg-brand";
  const label = EDITORIAL_TYPE_LABELS[post.type] || post.type;

  return (
    <div className="bg-white border-2 border-brand/20 hover:border-brand transition-colors overflow-hidden flex flex-col h-full relative group">
      <Link href={href} className="flex-1 flex flex-col">
        <div className="aspect-video bg-light-surface overflow-hidden relative">
          {post.coverImage ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-dark-surface">
              <span className="text-dark-muted text-4xl">✍️</span>
            </div>
          )}
          <div className="absolute top-2 left-2 flex gap-1">
            <span className="text-xs font-medium px-2 py-0.5 bg-brand text-white">
              Editorial
            </span>
            <span className={`text-xs font-medium px-2 py-0.5 ${colour} text-white`}>
              {label}
            </span>
          </div>
        </div>
        <div className="p-4 flex-1 flex flex-col">
          ...
          {post.excerpt && (
            <p className="text-light-muted text-sm mt-2 line-clamp-2">{post.excerpt}</p>
          )}
      ...
      <div className="p-4 border-t border-light-border">
        <EditorialShareButtons post={post} section={section} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="mb-8">
      ...
      >
        ...
        <span className="text-light-muted text-sm">({items.length})</span>
      </button>
      {open && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-fr">
          {items.map((item) =>
            item._isEditorial ? (
              <EditorialTeaserCard key={`editorial-${item.id}`} post={item as EditorialTeaser} />
            ) : (
              <NewsCard key={item.id} article={item as NewsArticle} />
            )
          )}
        </div>
      )}
...
export default function NewsPage() {
  ...
  const [teasers, setTeasers] = useState<EditorialTeaser[]>([]);
  ...
  // Merge regular articles and teasers into a single timeline
  const groupedByMonth = useMemo(() => {
    ...
    regular.forEach((a) => {
      ...
      groups[key].push({ ...a, _isEditorial: false });
    });

    // Add editorial teasers
    teasers.forEach((t) => {
          ...
          {groupedByMonth.length === 0 ? (
            ...
            groupedByMonth.map(([month, items]) => (
              <MonthGroup
                ...
                month={month}
                items={items}
                defaultOpen={month === currentMonthKey}
              />
            ))
          )}
        </div>
